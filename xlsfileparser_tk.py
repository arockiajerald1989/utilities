...
filename = 'TK.xlsx'  # Replace with your actual filename
...
df_merged = df.groupby(['SS ID', 'FullName'])['Hours'].sum().reset_index()
...
df_merged = df_merged[['FullName', 'SS ID'] + ['Hours', 'Regular Hours', 'Overtime Hours']]

# Optionally, write the merged DataFrame to a new Excel file
df_merged.to_excel('TK_Final.xlsx', index=False)
